...
public class JwtServiceTest {
    ...
    void testCreateTokenIndirectly() {
      ...
      long expirationTime = 3600000L; // 1 heure
      Map<String, Object> claims = new HashMap<>();
      String expectedToken = "accessToken";

      // Simuler la création d'un token
      given(jwtService.createToken(new HashMap<>(), userName, expirationTime))
              .willReturn(expectedToken);

      // Appeler la méthode createToken (publique maintenant)
      String actualToken = jwtService.createToken(claims, userName, expirationTime);

      // Vérification que le token généré est correct

...
public class JwtServiceTest {
    ...
    void testExtractClaim() {
        // Créer un mock des Claims
        Claims mockClaims = mock(Claims.class);

        // Simuler la valeur de "subject" (nom d'utilisateur) dans les Claims
        String expectedUserName = "username";
        when(mockClaims.getSubject()).thenReturn(expectedUserName);

        // Créer un mock de la méthode extractAllClaims dans JwtService
        JwtService jwtService = mock(JwtService.class);
        when(jwtService.extractAllClaims(anyString())).thenReturn(mockClaims);

        // Simuler la fonction qui sera passée à extractClaim
        Function<Claims, String> claimsResolver = Claims::getSubject;

        // Appeler la méthode extractClaim
        String actualUserName = jwtService.extractClaim("validToken", claimsResolver);

        // Vérification que la valeur extraite est correcte
        assertEquals(expectedUserName, actualUserName, "Le nom d'utilisateur extrait doit être correct");
    }
    @Test
    void testExtractUsername() {
        ...
        given(jwtService.extractClaim(eq(token), claim::getSubject()))
